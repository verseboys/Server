...
//    get_list()
    ...
    CKEDITOR.replace("ask_area", {customConfig: "../../progress_text_setting.js"}, CKEDITOR.on("instanceCreated", function (n) {
    }))
    ...
    if(getUrlParam('article')){
        article_type=false;
        $.ajax({
            url:u+'/supervisor/alter_news/'+getUrlParam('article')+'/',
            type:'get',
            success:function (data) {
                if(data.code!==200){
                    alert(data.message);
                ...
                CKEDITOR.instances.ask_area.setData(con)
            }
        })
    }else {
...
$(document).ready(function () {
    ...
    function got_data(){
        test_type=1
        recomm_obj={}
        $('.search-test').addClass('select').siblings().removeClass('select');
        $.ajax({
            url:u+'/supervisor/news_related_reading_article_list/'+ getUrlParam('article')+'/',
            type:'get',
            success:function(data){
                var list = data.data.news_realted_reading,str='';
                if(list) {
                    for (var i = 0; i < list.length; i++) {
                        var iii = list[i].article_id
                        str += '<div class="progress-list" style="line-height: 40px">\n' +
                            '                        <div class="progress-title" title="' + list[i].related_article + '">' + list[i].related_article + '</div><div class="progress-recom">' +

                            '<span class="tag-if-show checked" p_id="' + list[i].id + '" type="' + list[i].type + '" style="margin-right: 0" push_id="' + list[i].related_reading_article + '"></span>' +
                    ...
                    str+='<div class="tag-class-box" style="text-align: center; line-height: 100px;">暂无文章</div></div>'
                }
                $('.progrss_recommen_list').html(str)
                console.log(recomm_obj)
            }
        })
    }
    //搜索列表
    function got_list() {
        if(!$('.progress-search-box input').val()){
            alert('请输入搜索关键词')
            return
        }
        $('.search-result').addClass('select').siblings().removeClass('select');
        test_type=2
        $.ajax({
            url:u+'/supervisor/search_methods_and_news_articles/'+1+'/',
            type:'post',
            data:{
              search_key_words:$('.progress-search-box input').val()
            },
            success:function (data) {
                var str = '' ;

                if (data.code !== 200) {
                    $('#more-article').html('<div class="tag-class-box" style="text-align: center; line-height: 100px;">暂无文章</div></div>')

                    return
                }
                var list = data.data.articleModel;

                if (list) {
                    for(var i=0;i<list.length;i++){
                        var iii=list[i].id
                        str+='<div class="progress-list" style="line-height: 40px">\n' +
                            '                        <div class="progress-title" title="'+list[i].title+'">'+list[i].title+'</div><div class="progress-recom">'
                        console.log(iii,recomm_obj[iii])
                        if(recomm_obj[iii]===true) {
                            str += '<span class="tag-if-show checked" p_id="' + list[i].id + '" type="' + list[i].type + '" style="margin-right: 0" ></span>'
                        }else {
                            str += '<span class="tag-if-show" p_id="' + list[i].id + '" type="' + list[i].type + '" style="margin-right: 0" ></span>'
                        }
                           str+= '</div>\n' +
                            ' </div> '
                    }
    ...
    $(document).on('click', '.progress-recom .tag-if-show', function () {
        ...
        var type=$(this).attr('type');
            if ($(this).hasClass('checked')) {
                if(test_type===1) {
                    $.ajax({
                        url: u + '/supervisor/delete_news_related_reading/' + p + '/',
                        type: 'get',
                        data: {},
                        success: function (data) {
                            got_data()
                ...
                $.ajax({
                    url: u + '/supervisor/add_news_related_reading_articles/' + getUrlParam('article') + '/' + p + '/' + type + '/',
                    type: 'get',
                    data: {},
                    success: function (data) {
                        if(data.code==200){
    ...
    //搜索
    $(document).on('click','.progress-search-box span',function(){
        got_list()
    })
    $(document).on('keyup','.progress-search-box input',function(e){
        if (e.keyCode === 13) {
            $(this).blur()
            got_list()
        }
    ...
    //推荐
    $(document).on('click','.search-test',function () {
        got_data()
    })
})
