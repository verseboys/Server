    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < list.length; i++) {
                var p=list[i].id;
                var t=list[i].section
                model_tit_obj[p]=true;
                model_tit_t[p]=t
            }
        }
    })
...
if(getUrlParam('article')){
        article_type=false;
        $('.add-article').removeClass('hide')
        $('.add-model').removeClass('hide')
        $.ajax({
            url:u+'/supervisor/alter_method_article/'+getUrlParam('article')+'/',
            type:'get',
            success:function (data) {
                    if(data.code!==200){
                        alert(data.message);
                ...
                data=data.data;
                $('.title_box input').val(data.title);
                $('.abstract_box textarea').val(data.summary)
                var list= methods_list;
                var chil
                var str=' <span><select id="artticle-classify-l">'
                for(var i=0;i<list.length;i++){
                    if(data.category_id===list[i].id) {
                        str += '<option value="' + list[i].id + '" selected>' + list[i].category + '</option>'
                    }else {

                        if(list[i].relevance_category){
                            var chi=list[i].relevance_category
                            for(var j=0;j<chi.length;j++){
                                if(data.category_id===chi[j].id) {

                                    chil=list[i].relevance_category
                                    str += '<option value="' + list[i].id + '" selected>' + list[i].category + '</option>';
                                }
                            }
                        }else{
                            str += '<option value="' + list[i].id + '">' + list[i].category + '</option>';
                        }
                ...
                str+='</select></span>';
                str+='<span id="artticle-classify-r">'
                if(chil){
                    str+='<select >'
                    for(var k=0;k<chil.length;k++){
                        if(data.category_id===chil[k].id) {
                            str += '<option value="' + chil[k].id + '" selected>' + chil[k].category + '</option>'
                        }else {
                            str += '<option value="' + chil[k].id + '">' + chil[k].category + '</option>'
                        }
                    }
                    str+='</select>'
                }
                str+='</span>'
                $('#artticle-classify').html(str)
        $('.flex-right .tag-if-show[p='+data.rank+']').trigger('click')
        $('.flex-right .tag-if-radius[p='+data.status+']').trigger('click')
$('.public_time_input').val(data.create_time.replace('T',' '));
                $('.medel-up-file .medel-up-file-text').html(data.file_name===null?'':data.file_name)
                data.thumbnail?$('.pc_banner').css('background-image','url('+u+'/images/article_images/'+data.thumbnail+')'):'';
                img_url= data.thumbnail ? data.thumbnail:"";
                data.app_thumbnail ? $('.web_banner').css('background-image', 'url(' + u + '/images/article_images/' + data.app_thumbnail + ')') : '';
                img_web_url = data.app_thumbnail ? data.app_thumbnail : ''
                file_o_name= data.file_name === null ? '' : data.file_name

              var con=data.content,con_str=''
                for(var con_item=0;con_item<con.length;con_item++){
                  var p=con[con_item].section_id
                    model_tit_obj[p] = false;
con_str+='<div  class="model-mothods-box" p_ord="'+con_item+'">\n' +
    '            <div class="modek-head">\n' +
    '                <div class="modek-head-left" p_id="'+con[con_item].section_id+'">'+con[con_item].section+'</div>\n' +
    '                <div  class="modek-head-right"><span class="edit" p_id="'+con[con_item].section_id+'" p_ord="'+con_item+'" con_id="'+con[con_item].content_id+'">修改</span> / <span class="del" p_id="'+con[con_item].section_id+'" con_id="'+con[con_item].content_id+'">删除</span></div>\n' +
    '            </div>\n' +
    '            <div class="model-meth-viwe">\n' +
    '               <span>顺序：'+con[con_item].rank+'</span><span>登录后是否可查看：'+(con[con_item].is_login===1?"是":"否")+'</span>\n' +
    '            </div>\n' +
    '            <div class="model-meth-content">'+con[con_item].content+'</div>\n' +
    '        </div>';
                    model_len=con_item
                }
                model_len++
                $('.sunmit').parent().before(con_str);
            }
        })
}else {
    var list= methods_list;
    ...
    str+='</select></span>';
    var chil=list[0].relevance_category;
    str+='<span id="artticle-classify-r">'
    if(chil){
        str+='<select >'
        for(var j=0;j<chil.length;j++){
            str+='<option value="'+chil[j].id+'">'+chil[j].category+'</option>'
        }
        str+='</select>'
    }
    str+='</span>'
    ...
    $(document).on('click','.flex-right .tag-if-show',function () {
        class_tag=$(this).attr('p')
    ...
    $(document).on('change','#artticle-classify-l',function () {
        var v=$(this).val()
        for(var i=0;i<methods_list.length;i++){
            if(methods_list[i].id===Number(v)){
                var chil=methods_list[i].relevance_category;
                var str='';
                if(chil){
                    str+='<select >'
                    for(var j=0;j<chil.length;j++){
                ...
                $('#artticle-classify-r').html(str)
            }
        }
    })
    $(document).on('change','#artticle-classify-r select',function () {
    ...
    $(document).on('click','.web_banner',function () {
        $('#up_web_img').trigger('click')
    })
    //图片
    $(document).on('change','#up_img',function () {
        ...
        reader.onload = function(e){
            ...
            $.ajax({
                type: 'post',
                url: u+'/extends/image_storage/article_images/',
                data: oMyForm,
                dataType: 'json',
                contentType: false,
                processData: false,
                success: function(e) {
                    if(e.code===200){
                        img_url=e.data;
                        $('.pc_banner').css('background-image','url('+u+'/images/article_images/'+e.data+')')
                    }else {
    ...
    $(document).on('change', '#up_web_img', function () {
        if (!this['value'].match(/.jpg|.gif|.png|.bmp|.JPEG|svg|/i)) {
            alert('请选择图片格式上传')
            return
        }
        ...
        reader.onload = function (e) {
            var oMyForm = new FormData();
            oMyForm.append("imgFile", $('#up_web_img')[0].files[0]);
            $.ajax({
                type: 'post',
                url: u + '/extends/image_storage/article_images/',
                data: oMyForm,
                ...
                success: function (e) {
                    if (e.code === 200) {
                        img_web_url = e.data;
                        $('.web_banner').css('background-image', 'url(' + u + '/images/article_images/' + e.data + ')')
                    } else {
                        console.log(e.message)
                    }
                    $('#up_web_img').val('')
    ...
    $(document).on('click','.sunmit',function () {
        ...
        var artticle_classify='';
        if($('#artticle-classify-r select')[0]){
            artticle_classify=$('#artticle-classify-r select').val()
        }else {
            artticle_classify=$('#artticle-classify-l').val()
        }
        var str='';
        if(!title){
            str+='请输入标题\n';
        ...
        var obj={
            ...
            "summary":article,
            "thumbnail":img_url,
            "category_id":artticle_classify,
            "rank":class_tag,
            "status":class_t,
            "file_name":file_o_name,
            "create_time":time,
            "app_thumbnail": img_web_url
        }

        if(!getUrlParam('article')) {

            $.ajax({
                url: u + '/supervisor/add_method_article_info/',
                type: 'post',
                data: obj,
                success: function (data) {
                    if (data.code === 200) {
                        window.location.search='article='+data.data.id
                    }
                }
            })
        }else {
            //
            // if ($('.model-mothods-box').length) {
            //     $('.model-mothods-box').each(function (i) {
            //         var p = $(this).find('.modek-head-left').attr('p_id');
            //         var content = $(this).find('.model-meth-content').html();
            //         console.log('"'+content+'"')
            //         var r = $(this).find('.model-meth-viwe span').eq(0).html().replace('顺序：', '');
            //         var show = $(this).find('.model-meth-viwe span').eq(1).html().replace('登录后是否可查看：', '');
            //         show === '是' ? show = "1" : show = "0";
            //         obj[p] = '{"is_login":"' + show + '","content":"' +content + '", "rank":"' + r + '"}'
            //     })
            // }
            $.ajax({
                url: u + '/supervisor/alter_method_article/' + getUrlParam('article') + '/',
                ...
                success: function (data) {
                    if(data.code===200){
                        window.location.href='/supervisor/methods.html'
                    }else {
                        alert(data.message)
                    }
                }
...
//类型
    ...
    var con
    //附件
    $(document).on('change','#model-up-file',function () {
        var reader = new FileReader();
        var oMyForm;
        var _self = this;
        reader.readAsDataURL(this.files[0]);
        reader.onload = function(e){
            var oMyForm = new FormData();
            oMyForm.append("file_info", $('#model-up-file')[0].files[0]);
            $.ajax({
                type: 'post',
                url: u+'/extends/upload_file/article/',
                data: oMyForm,
                dataType: 'json',
                contentType: false,
                processData: false,
                success: function(e) {
                    if(e.code===200){
                        file_o_name=e.data;
                        $('.medel-up-file .medel-up-file-text').text($('#model-up-file')[0].files[0].name)
                    }else {
                        $('#model-up-file').val('');
                        alert(e.message)
                    }
    ...
    //计算文本高度
    function model_article_height() {
        var btn_hei=$('.flex-article-btn').height()
        var out_hei=$('.flex-box').height()-btn_hei-49
        var tit_hei=$('.flex-add-article-title').height()+29
        var cel_hei=$('.flex-add-article-concel').height()+28
        $('.model-article-content').height(out_hei-tit_hei-cel_hei)
        $('#cke_1_contents').height(out_hei-tit_hei-cel_hei-42)
    }
    $(window).resize(function () {
        if(!$('.flex-outer').hasClass('hide')){
            model_article_height()
        }
    })
    //取消模块
    $(document).on('click','.flex-article-btn .re-close',function () {
    ...
    $(document).on('click','.add-model',function () {
        ...
        model_article_height();
        var order_num=$('.model-mothods-box').length+1,order_str=''
        for(var order_item=1;order_item<=order_num;order_item++){
            if(order_num===order_item){
                order_str+='<option value="'+order_item+'" selected>'+order_item+'</option>'
            }else {
                order_str+='<option value="'+order_item+'">'+order_item+'</option>'
            }
        }
        $('.flex-add-article-concel-order select').html(order_str);
        var tit_str=''
        for(var tit_item=0;tit_item < model_tit_list.length;tit_item++){
            var p=model_tit_list[tit_item].id
            if(model_tit_obj[p]){
                tit_str+='<option value="'+p+'">'+model_tit_list[tit_item].section+'</option>'
            }
        }
        $('.flex-add-article-concel-model select').html(tit_str);


    })
    $(document).on('click','.flex-add-article-concel-show .tag-if-show',function () {
   ...
   try{
    ...
    //这留有富文本内容提取方法
    if(CKEDITOR.instances.ask_area.getData().length>0) {
        var model_tit_id = $('.flex-add-article-concel-model select').val()
        var model_tit_text = model_tit_t[model_tit_id]
        var is_show = $('.flex-add-article-concel-show .tag-if-show').hasClass('checked') ? 1 : 0;
        var content = CKEDITOR.instances.ask_area.getData()
        model_tit_obj[model_tit_id] = false;
        var str=''
        if (model_type === 'add') {
            $.ajax({
                url:u+'/supervisor/add_method_article_content/',
                type:'post',
                data:{
                    article_id:getUrlParam('article'),
                    content:content,
                    article_content_section_id:model_tit_id,
                    rank:$('.flex-add-article-concel-order select').val(),
                    is_login:$('.flex-add-article-concel-show .tag-if-show').hasClass('checked') ? 1 : 0
                },
                success:function (data) {
                    if(data.code===200){
                        window.location.reload()
                        // str+='<div  class="model-mothods-box" p_ord="'+model_len+'">'
                        //
                        // str += '<div class="modek-head">';
                        // str += '<div class="modek-head-left" p_id="' + model_tit_id + '">' + model_tit_text + '</div>'
                        // str += '<div  class="modek-head-right"><span class="edit" p_id="' + model_tit_id + '" p_ord="'+model_len+'">修改</span> / <span class="del" p_id="' + model_tit_id + '">删除</span></div></div>'
                        // str += ' <div class="model-meth-viwe"><span>顺序：';
                        // str += $('.flex-add-article-concel-order select').val()
                        // str += '</span><span>登录后是否可查看：'
                        // str += $('.flex-add-article-concel-show .tag-if-show').hasClass('checked') ? '是' : '否';
                        // str += '</span></div>'
                        // str += '<div class="model-meth-content">'
                        // str += content
                        // str += '</div>'
                        // str += '</div>'
                        // $('.sunmit').parent().before(str);
                    }
                }
            })
    }else if(model_type==='edit'){
        $.ajax({
            url:u+'/supervisor/alter_method_article_content/',
            type:'post',
            data:{
                article_id:getUrlParam('article'),
                content:content,
            ...
            success:function(data){
                if(data.code===200){
                    str += '<div class="modek-head">';
                    str += '<div class="modek-head-left" p_id="' + model_tit_id + '">' + model_tit_text + '</div>'
                    str += '<div  class="modek-head-right"><span class="edit" p_id="' + model_tit_id + '" p_ord="'+model_t_len+'" con_id="'+con+'">修改</span> / <span class="del" con_id="'+con+'" p_id="' + model_tit_id + '">删除</span></div></div>'
                    str += ' <div class="model-meth-viwe"><span>顺序：';
                    str += $('.flex-add-article-concel-order select').val()
                    str += '</span><span>登录后是否可查看：'
                    str += $('.flex-add-article-concel-show .tag-if-show').hasClass('checked') ? '是' : '否';
                    ...
                    str += content
                    str += '</div>'
                    $('.model-mothods-box[p_ord="'+model_t_len+'"]').html(str);
                    model_tit_obj[model_old_p]=true;
                }
            }
        ...
        $('.re-close').trigger('click');
$('.model-mothods-box').length===model_tit_list.length?$('.add-model').addClass('hide'):$('.add-model').removeClass('hide')
    ...
    $(document).on('click','.edit',function () {
        model_type='edit'
        model_t_len=$(this).attr('p_ord')
        var sel_id=$(this).attr('p_id')
        con=$(this).attr('con_id')
        model_old_p=sel_id
var ord_n=$(this).parents('.modek-head').next().find('span').eq(0).html().replace('顺序：','')
var ord_show=$(this).parents('.modek-head').next().find('span').eq(1).html().replace('登录后是否可查看：','')
        var content=$(this).parents('.modek-head').next().next().html()
        $('.flex-outer').removeClass('hide');
        model_article_height();

        var order_num=$('.model-mothods-box').length+1,order_str=''
        for(var order_item=1;order_item<order_num;order_item++){
            if(Number(ord_n)===order_item){
                order_str+='<option value="'+order_item+'" selected>'+order_item+'</option>'
            }else {
                order_str+='<option value="'+order_item+'">'+order_item+'</option>'
            }
        ...
        for(var tit_item=0;tit_item < model_tit_list.length;tit_item++){
            var p=model_tit_list[tit_item].id
            if(Number(sel_id)===p){
                tit_str+='<option value="'+p+'" selected>'+model_tit_list[tit_item].section+'</option>'
            }else if(model_tit_obj[p]){
                tit_str+='<option value="'+p+'">'+model_tit_list[tit_item].section+'</option>'
            }
        }
        $('.flex-add-article-concel-model select').html(tit_str);

        ord_show==='是'?$('.flex-add-article-concel-show .tag-if-show').addClass('checked'):$('.flex-add-article-concel-show .tag-if-show').removeClass('checked')
        //富文本修改内容方法
            CKEDITOR.instances.ask_area.setData(content)

    })
    //删除
    $(document).on('click','.del',function () {
        var p=$(this).attr('p_id');
        var con=$(this).attr('con_id');
        model_tit_obj[p]=true;
        var self=this
        $.ajax({
            url:u+'/supervisor/delete_method_article_content/',
            type:'post',
            data:{
                article_id:getUrlParam('article'),
                content_id:con
            ...
            success:function (data) {
                if(data.code===200){
                    $(self).parents('.model-mothods-box').remove()
                }else{
                    alert(data.message)
                }
            }
...
$(document).ready(function () {
    var page='', item='',url="/supervisor/related_reading_method_article_list/" + getUrlParam('article') + '/',ttp="RECOM",page_num;
    var recomm_obj={}
    if(getUrlParam('article')) {
        $.ajax({
            url: u + url + page + item,
                ...
                if (data.data.articles) {
                    ...
                    if (ttp === "RECOM") {
                        str += '                        <div class="me-class-left" style="padding-left: 20px;">标题</div>\n' +
                            ...
                            '                            <div class="me-class-6" style="width: 25%;">是否关联</div>\n' +
                            '                        </div>\n' +
                            '                    </div>'
                    } else {
                        str += '                    <div class="me-class-left" style="padding-left: 20px;width: 44%;">标题</div>\n' +
                            ...
                            '                        <div class="me-class-6" style="width: 38%;">是否关联</div>\n'
                    }
                    str += '                    </div>\n' +
    ...
    function got_data(){
        $.ajax({
            ...
            success:function(data){
                ...
                var str='<span class="checked" p="RECOM">相关阅读</span>',list=data.data.category_list;
                for(var i=0;i<list.length;i++){
...
//            $('#panel_content').html(str)
    ...
    $(document).on('click','.recomm_title span',function () {
        ...
        if(item==='RECOM'){
            item='';
            page=''
            url="/supervisor/related_reading_method_article_list/" + getUrlParam('article') + '/';
    ...
    $(document).on('click', '.more-article .tag-if-show', function () {
        ...
        if ($(this).hasClass('checked')) {
            $.ajax({
                url: u + '/supervisor/delete_related_reading_method_article/' + getUrlParam('article') + '/',
                type: 'post',
                data: {
                    related_reading_article_id: p,
                },
                success: function (data) {
                    if (data.code !== 200) {
                        alert(data.message)
            ...
            $.ajax({
                url: u + '/supervisor/add_related_reading_method_article/' + getUrlParam('article') + '/',
                type: 'post',
                data: {
                    related_reading_article_id: p,
                },
...
//        ary.push()
        ...
        obj[p_p] = t_n;
        $.ajax({
            url: u + '/supervisor/alter_related_reading_method_article_arrange/'+ getUrlParam('article') + '/',
            type: 'post',
            data: obj,
            success: function (data) {
                if (data.code !== 200) {
